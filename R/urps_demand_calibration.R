...
.URPS_DEMAND_CALIBRATION_STATES <- c(
  ...
  "example")          # a synthetic, typed example of the format
...
validate_urps_demand_params <- function(x) {
  ...
  if (nrow(x) != length(.URPS_DEMAND_SERVICE_TYPES))
    stop(sprintf("[validate_urps_demand_params] expected %d service rows, got %d.",
                 length(.URPS_DEMAND_SERVICE_TYPES), nrow(x)), call. = FALSE)
  ...
  if (!setequal(x$service_type, .URPS_DEMAND_SERVICE_TYPES))
    stop("[validate_urps_demand_params] `service_type` must be exactly: ",
         paste(.URPS_DEMAND_SERVICE_TYPES, collapse = ", "), ".", call. = FALSE)
  ...
  if (length(status) != 1L || !status %in% .URPS_DEMAND_CALIBRATION_STATES)
    stop("[validate_urps_demand_params] `calibration_status` must be a single value in {",
         paste(.URPS_DEMAND_CALIBRATION_STATES, collapse = ", "), "}.", call. = FALSE)
  ...
  if (status == "not_calibrated") {
    ...
    if (!all_fin)
      stop(sprintf("[validate_urps_demand_params] calibration_status '%s' requires every beta to be finite (no NA).",
                   status), call. = FALSE)
