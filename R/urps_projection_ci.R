...
urps_projection_ci <- function(project_fn,
  ...
  headcount_mat <- matrix(NA_real_,
    nrow = B, ncol = length(combo_keys),
    dimnames = list(NULL, combo_keys)
  )
  fte_mat <- matrix(NA_real_,
    nrow = B, ncol = length(combo_keys),
    dimnames = list(NULL, combo_keys)
  )

  for (b in seq_len(B)) {
    pd <- urps_ci_param_draw(
      retirement_sigma_sd = retirement_sigma_sd,
