...
`NAMESPACE` is maintained by hand to match the `@export` tags in `R/`; keep the
two in sync when adding an export. When you promote a new constant, add its
provenance and a fail-loud `stopifnot()` block in the same commit, and record the
change in [`NEWS.md`](NEWS.md).
