# Adversarial + semantic tests for the promoted accessibility-disparity
# statistics in R/accessibility_stats.R.
#
# PORTED FROM isochrones tests/testthat/test-accessibility-stratification.R
# (at isochrones 7ac5191c), which is the suite R/accessibility_stats.R already
# names as guarding this code upstream. The functions were promoted here
# verbatim so isochrones and twostep would share one definition, but the tests
# that guarded them stayed behind -- mufflyaccess covered them with a 30-line
# smoke test while the real property, regression and edge-case coverage lived
# in the origin repo. This closes that gap.
#
# The centerpiece is a REGRESSION GUARD for the clamp/filter bug caught upstream
...
# tracts of sparse groups (American Indian/Alaska Native, Native
# Hawaiian/Pacific Islander) and pushing the point estimate OUTSIDE its own
# confidence interval.
#
# Adapted only where it had to be: the origin sources the implementation file
# directly, whereas these functions are exported here, so the source() call is
# dropped. The assertions are unchanged.
...
test_that("weighted_mean_all equals the naive point mean for non-negative weights", {
  a <- c(0.1, 0.5, 0.9)
  w <- c(0, 100, 300) # a zero-weight element must not matter
...
test_that("weighted_mean_all is bounded by [min(a), max(a)] under non-negative weights", {
  ...
  for (i in 1:50) {
    ...
    expect_gte(m, min(a) - 1e-12)
    expect_lte(m, max(a) + 1e-12)
  }
})
...
test_that("rurality_from_ruca uses the USDA cut at 3|4 (Metro 1-3, Rural 4-10)", {
  expect_equal(
    rurality_from_ruca(c(1, 2, 3, 4, 7, 10)),
...
test_that("annual_trend recovers the sign and magnitude of a synthetic trend", {
  ...
  # suppressWarnings: lm() emits "essentially perfect fit" on a noiseless
  # synthetic line. That is a property of the fixture, not a signal about
  # annual_trend(), and letting it through would leave permanent WARNs in the
  # suite for a real one to hide behind. Assertions are unchanged.
  tr <- suppressWarnings(annual_trend(yr, declining))
  expect_equal(unname(tr["slope"]), -0.2, tolerance = 1e-8)
  expect_lt(unname(tr["p"]), 1e-6) # a perfect line is highly significant
  flat <- suppressWarnings(annual_trend(yr, rep(5, length(yr))))
...
test_that("a pooled weighted mean lies between the two group means (pooling law)", {
  ...
  for (i in 1:40) {
    ...
    w1 <- runif(10, 1, 5)
    a2 <- runif(10, 2, 3)
    w2 <- runif(10, 1, 5) # group 2 strictly higher access
    m1 <- weighted_mean_all(a1, w1)
    m2 <- weighted_mean_all(a2, w2)
...
test_that("rurality_from_ruca returns NA for RUCA 99 and other uncoded values", {
  ...
  expect_equal(
    rurality_from_ruca(c(1, 99, 7, 0, 3)),
...
test_that("wired wmean/zshare reproduce the historical inline estimators (non-negative ACS weights)", {
  ...
  zshare_old <- function(a, w) {
    ok <- !is.na(w) & w > 0
    if (!any(ok)) {
      return(NA_real_)
    }
    sum(w[ok][a[ok] == 0]) / sum(w[ok])
  }
  # the wrappers as defined in scripts/stratify_allyears_access.R
  wmean_new <- function(x, w) {
    bad <- is.na(x) | is.na(w)
    x[bad] <- 0
    w[bad] <- 0
    weighted_mean_all(x, w)
  }
  zshare_new <- function(a, w) {
    bad <- is.na(a) | is.na(w)
    a[bad] <- 0
    w[bad] <- 0
    zero_access_share(a, w) / 100
  }
