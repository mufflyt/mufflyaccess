...
semantic_mutations <- list(
  ...
  list(name = "duplicate count key",
       mutate = function(p) mutate_counts(p, function(d)
         rbind(d, d[d$year == 2023 & d$measure == "board_certified_active" &
                    d$geography == "national" & d$board_pathway == "ABOG", ])),
       error = "duplicate"),
  list(name = "missing pathway row",
       mutate = function(p) mutate_counts(p, function(d)
  ...
  list(name = "2025 snapshot mislabeled as a 2024 board-certified year",
       mutate = function(p) mutate_counts(p, function(d) {
         r <- d[d$year == 2023 & d$measure == "board_certified_active" &
                d$geography == "national", ]; r$year <- 2024L; rbind(d, r) }),
       error = "2013.*2023|window|unsupported"),
  list(name = "altered snapshot date in the manifest",
       mutate = function(p) edit_manifest(p, function(m) { m$snapshot_date <- "2020-01-01"; m }),
       error = "snapshot"),
  list(name = "release-contract canonical cell disagrees with counts",
       mutate = function(p) edit_contract(p, function(ct) { ct$canonical$n_active <- 1300L; ct }),
