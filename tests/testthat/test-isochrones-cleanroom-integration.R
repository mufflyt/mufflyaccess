...
test_that("served 2023 national count reconstructs from provider rows", {
  ...
  expected <- sum(providers$certification_year <= 2023L &
                    (is.na(providers$retirement_year) |
                       providers$retirement_year > 2023L))
...
test_that("exactly seven providers are first certified after 2023", {
  path <- real_isochrones_artifact_path()
  providers <- read_provider_parquet(path)
  expect_equal(sum(providers$certification_year >= 2024L), 7L)
})
