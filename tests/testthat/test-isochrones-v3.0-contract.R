...
test_that("an unsupported major contract version fails closed", {
  ...
  edit_manifest(path, function(x) { x$contract_version <- "4.0.0"; x })
...
test_that("the retired v2.x contract is no longer accepted as current", {
  path <- copy_urps_fixture("isochrones-v3.0.0")
  edit_manifest(path, function(x) { x$contract_version <- "2.1.0"; x })
  edit_contract(path, function(x) { x$contract_version <- "2.1.0"; x })
...
test_that("a supported minor bump is accepted", {
  path <- copy_urps_fixture("isochrones-v3.0.0")
  edit_manifest(path, function(x) { x$contract_version <- "3.1.0"; x })
  edit_contract(path, function(x) { x$contract_version <- "3.1.0"; x })
  expect_no_error(validate_urps_artifact(path))
})

test_that("invalid semantic versions fail closed", {
  path <- copy_urps_fixture("isochrones-v3.0.0")
  edit_manifest(path, function(x) { x$contract_version <- "3.0"; x })   # not x.y.z
...
test_that("manifest and release-contract versions may not disagree", {
  path <- copy_urps_fixture("isochrones-v3.0.0")
  edit_manifest(path, function(x) { x$contract_version <- "3.2.0"; x })  # contract stays 3.0.0
