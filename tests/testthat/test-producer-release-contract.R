...
test_that("bundled counts match the producer release contract (v2.1.0 shape)", {
  ...
  expect_equal(contract$contract_version, "2.1.0")

  # canonical 2023 estimand = board_certified_active / national / ABOG_PLUS_ABU = 1332
  expect_equal(contract$canonical$n_active, 1332L)
  expect_equal(urps_count(2023, "board_certified_active", "national", TRUE),
