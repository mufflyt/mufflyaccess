...
test_that("standardize_state_name maps names/codes/territories both directions", {
  expect_equal(standardize_state_name(c("colorado", "TX", "Puerto Rico"), output = "abbr"),
               c("CO", "TX", "PR"))
  expect_equal(standardize_state_name(c("co", "tx"), output = "name"),
               c("Colorado", "Texas"))
...
test_that("fingerprint_files hashes content, is order-independent, NA when absent", {
  ...
  f1 <- file.path(d, "a.R"); f2 <- file.path(d, "b.R")
  writeLines("x <- 1", f1); writeLines("y <- 2", f2)
