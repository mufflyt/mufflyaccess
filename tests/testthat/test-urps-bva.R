...
test_that("board_certified_active year window: 2012 | 2013 .. 2023 | 2024", {
  ...
  expect_error(
    urps_count(2024, "board_certified_active", "national", TRUE),
    "unsupported year|2013"
  ) # max + 1
...
test_that("supply_headcount non-negativity boundary: 0 passes, just-below fails", {
  ...
  expect_error(
    validate_urps_projection(base(supply_headcount = -1e-9)), # just below
    "non-negative"
  )
})
...
test_that("95% bounds bracket INCLUSIVELY (lower<=point<=upper)", {
  ...
  expect_error(
    validate_urps_projection(base(lower_95 = sh + 1e-6, upper_95 = sh + 5)), # lower just above
    "bounds do not bracket"
  )
  expect_error(
    validate_urps_projection(base(lower_95 = sh - 5, upper_95 = sh - 1e-6)), # upper just below
    "bounds do not bracket"
  )
})
...
test_that("supply_clinical_fte bound [0, headcount] is INCLUSIVE at both ends", {
  ...
  expect_error(
    validate_urps_projection(base(supply_headcount = sh, supply_clinical_fte = sh + 1e-9)),
    "cannot exceed"
  ) # just above headcount
  expect_error(
    validate_urps_projection(base(supply_headcount = sh, supply_clinical_fte = -1e-9)),
    "non-negative"
  ) # just below 0
})
...
test_that("entrants / exits non-negativity boundary: 0 passes, just-below fails", {
  ...
  expect_error(
    validate_urps_projection(base(entrants = -1e-9, exits = 0, net_change = -1e-9)),
    "non-negative"
  )
})
