...
test_that("urps_departures() fails loud when no observed artifact is configured", {
  old <- options(mufflyaccess.urps_departures_path = NULL)
...
test_that("urps_exit_hazard_by_age_year() fails loud when unconfigured", {
  old <- options(mufflyaccess.urps_exit_hazard_path = NULL)
  on.exit(options(old), add = TRUE)
...
test_that("a configured departures artifact activates urps_departures()", {
  ...
  old <- options(mufflyaccess.urps_departures_path = dep_ex)
  on.exit(options(old), add = TRUE)
  ...
  expect_true(all(c(
    "provider_id", "exit_month", "exit_year",
...
test_that("urps_exit_counts() aggregates to annual observed counts", {
  ...
  ec <- urps_exit_counts()
  expect_true(all(c(
    "year", "n_departed", "n_retired", "n_retired_with_evidence",
    "retirement_status", "retirement_definition"
  ) %in% names(ec)))
...
test_that("urps_exit_hazard_by_age_year() serves the frozen hazard artifact", {
  ...
  old <- options(mufflyaccess.urps_exit_hazard_path = haz_ex)
  on.exit(options(old), add = TRUE)

  h <- urps_exit_hazard_by_age_year()
  expect_true(all(c(
    "age", "year", "n_at_risk", "n_exits", "exit_hazard",
    "hazard_source"
  ) %in% names(h)))
...
test_that("validate_urps_exit_evidence() rejects structural violations", {
  ev <- data.frame(
    provider_id = "1000000001", month = as.Date("2023-01-01"),
    source = "nppes", practice_evidence = TRUE
  )
  expect_error(validate_urps_exit_evidence(list(a = 1)), "must be a data.frame")
  expect_error(
    validate_urps_exit_evidence(ev[, setdiff(names(ev), "source")]),
    "missing column"
  )
  bad_month <- ev
  bad_month$month <- "not-a-date"
