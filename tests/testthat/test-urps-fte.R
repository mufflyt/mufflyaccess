...
test_that("the weight is age productivity x pathway clinical time x late factor", {
  ...
  a <- 50L; rel <- curve$rel_to_peak[match(a, curve$age)]
  ...
  expect_equal(urps_fte_weight(65L, "ABOG", late_from_age = 60L, late_factor = 0.75),
               curve$rel_to_peak[match(65L, curve$age)] * 0.75)
  expect_equal(urps_fte_weight(55L, "ABOG", late_from_age = 60L, late_factor = 0.75),
               curve$rel_to_peak[match(55L, curve$age)])            # below onset: unaffected
  ...
  expect_equal(urps_fte_weight(200L, "ABOG"),
...
test_that("effective FTE is additive and the scale anchors a reference cohort to headcount", {
  cs <- data.frame(age = c(45L, 62L, 45L, 62L),
  ...
  abog <- cs[cs$pathway == "ABOG", ]; abu <- cs[cs$pathway == "ABU", ]
  expect_equal(urps_effective_fte(abog, scale) + urps_effective_fte(abu, scale),
               urps_effective_fte(cs, scale), tolerance = 1e-9)      # additive across pathways
  expect_error(urps_fte_scale(data.frame(age = 45L, pathway = "ABOG", n = 0)),
               "non-positive")
})
