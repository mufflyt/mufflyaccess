...
test_that("urps_ci_param_draw returns a list with the required elements", {
  ...
  expect_true(all(c("retirement_sigma_sd", "retirement_shift",
                    "entrant_scale", "lfp_intercept_shift") %in% names(d)))
})
...
test_that("urps_projection_ci returns a data.frame with correct columns", {
  ci <- urps_projection_ci(.make_project_fn(), B = 10L, seed = 1L,
                            years = 2025:2027)
  expect_s3_class(ci, "data.frame")
  expect_true(all(c("year", "scenario_id",
                    "lower_headcount_95", "upper_headcount_95",
                    "lower_fte_95",       "upper_fte_95") %in% names(ci)))
})

test_that("urps_projection_ci returns one row per (year, scenario_id)", {
  ci <- urps_projection_ci(.make_project_fn(), scenarios = "baseline",
...
test_that("urps_projection_ci lower <= upper for headcount", {
  ci <- urps_projection_ci(.make_project_fn(), B = 20L, seed = 5L,
                            years = 2025:2030)
...
test_that("urps_projection_ci fte is NA when project_fn returns NA fte", {
  fn <- function(scenario_id, years, param_draw) {
    data.frame(year = years, scenario_id = scenario_id,
...
test_that("urps_projection_ci wider interval with larger entrant_cv", {
  fn <- .make_project_fn()
  ci_narrow <- urps_projection_ci(fn, B = 200L, seed = 1L, years = 2030:2030,
                                   entrant_cv = 0.01)
  ci_wide   <- urps_projection_ci(fn, B = 200L, seed = 1L, years = 2030:2030,
                                   entrant_cv = 0.30)
...
test_that("urps_projection_ci rejects B < 10", {
  expect_error(urps_projection_ci(.make_project_fn(), B = 5L),
               "B must be a single integer >= 10")
})

test_that("urps_projection_ci rejects non-function project_fn", {
  expect_error(urps_projection_ci("not_a_function", B = 10L),
...
test_that("urps_projection_ci errors if project_fn returns non-data.frame", {
  ...
  expect_error(urps_projection_ci(bad_fn, B = 10L, years = 2025:2026),
               "data.frame")
})
