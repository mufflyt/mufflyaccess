...
test_that("the schema is the canonical long-table spec", {
  ...
  expect_true(all(c(
    "year", "scenario_id", "certification_pathway", "geography_type",
    "supply_headcount", "supply_clinical_fte", "entrants", "exits",
    "net_change", "demand_clinical_fte", "gap_fte"
  ) %in% s$column))
...
test_that("baseline_tie catches a starting stock that disagrees with the SSOT", {
  d <- good()
  d$supply_headcount[d$scenario_id == "baseline" & d$year == 2025] <- 1306
  expect_error(
