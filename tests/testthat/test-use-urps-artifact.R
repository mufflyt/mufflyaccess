...
test_that("use_urps_artifact reads + validates an external artifact directory", {
  ext <- file.path(tempdir(), "urps_ext"); dir.create(ext, showWarnings = FALSE)
  bundled <- system.file("extdata", package = "mufflyaccess")
  file.copy(file.path(bundled, "urps_counts_by_year.csv"),
            file.path(ext, "urps_counts_by_year.csv"), overwrite = TRUE)
  ...
  expect_equal(urps_count(2023L, TRUE), 1339L)
...
test_that("use_urps_artifact rejects a missing directory", {
